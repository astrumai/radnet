...
def segmentation_loader_test(args):
    ...
    dataset = SegmentationLoader(train_path, labels_path, transform)
    # print("Shape of train array {} \nShape of labels array {}".format(train_arr.shape, label_arr.shape))

    temp = [transform(Image.fromarray((tiff.imread(train_path))[1]))]
    if torch.all(torch.eq((dataset.__getitem__(1)[0]), (temp[0]))):
        print("segmentation loader class test passed")
    else:
        raise AssertionError("The tensors are not the same")
