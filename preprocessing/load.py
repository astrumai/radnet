...
class DataTransformer(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.image_augmentation:
            images, labels = self.do_augment(images, labels)
        if self.image_augmentation is not None:
            images = self.image_transform(self._read_data(index))
            labels = self.image_transform(self._read_labels(index))

        return [images, labels]

    def do_augment(self, images, labels):
        images = np.array(images)
        labels = np.array(labels)
        res = self.image_augmentation(image=images, mask=labels)

        return [Image.fromarray(res['image']), Image.fromarray(res['mask'])]
